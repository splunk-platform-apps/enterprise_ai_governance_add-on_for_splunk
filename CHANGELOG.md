...
- Silenced the SpecFiles warnings splunkd logged at every startup on Splunk 9.x and 10.0 — one per input type, nine per boot. Two causes: the `python.required = 3.13` key in every `inputs.conf` stanza only exists in the newest Splunk spec files (removed; nothing consumed it, and `python.version = python3` remains), and the scheme-level default stanzas (`[anthropic_compliance]` etc.) were not declared in the add-on's `inputs.conf.spec`, which only carried the `[scheme://<name>]` forms (the shipped spec now declares both). Note: `README/inputs.conf.spec` is now maintained in `package/` rather than generated, so new inputs or argument changes in `globalConfig.json` must be mirrored there
